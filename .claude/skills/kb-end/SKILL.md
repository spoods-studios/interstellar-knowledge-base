---
name: kb-end
description: Close a session in the Expert-reviewed educational content - equations, articles, and translations. repo — append a dated entry to vault/learnings/sessions.md, capture any gray-area decision to vault/decisions/, and commit the working tree with a conventional-commit message (no push). Use --discard to skip all writes.
...
- User signals end of session: "done for now", "let's wrap", `/kb-end`.
...
`# Expert-reviewed educational content - equations, articles, and translations. — session log` heading if missing). Append:
